...
interface Look {
  ...
  texture?: number;
  /** Sky colour smoothing the look carries (EditParams.skySmooth). A SELECTION,
   *  not a whole-frame knob — it acts through the sky bitmap built at open and
   *  is inert on a frame with no sky. Creative, so it rides a saved look and is
...
/** The look's sky selection for the open photograph (skymap.ts), built at
...
function applyLook(name: keyof typeof LOOKS) {
  ...
  // The sky's colour smoothing is the look's to set, like the mixer: a false-
  // colour look is what amplifies the sky's residual thirteenfold (IR-SCIENCE
  // 4c-xxi), so the look that does that carries the stage that takes it back.
...
function draw() {
  ...
  raf = requestAnimationFrame(() => {
    ...
    syncSkyMap(); // the sky's smoothed chroma is a map of THIS edit
...
function wireVersionMenu() {
  ...
  const open = async () => {
    ...
    text.value = await buildDiagnostic(__APP_VERSION__, [
      ...
      { k: "Sky map", v: lastSkyMap ? `depth key ${lastSkyMap.key.toFixed(2)} from sky chroma ${lastSkyMap.chroma.toFixed(3)} (window ${SKY_DEPTH_CHROMA_LO}–${SKY_DEPTH_CHROMA_HI}) · palest sky texel ${lastSkyMap.paleTexel.toFixed(3)} single ${lastSkyMap.minTexel.toFixed(3)} (grey guard ${SKY_DEPTH_GREY_LO}–${SKY_DEPTH_GREY_HI} of the mean) · sky saturation ${(params.skySat ?? 0).toFixed(2)}` : "none built" },
...
/** THE STAGES A MASK CAN AIM TODAY (decision 030), as [button, bit] pairs.
