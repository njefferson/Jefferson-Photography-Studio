...
- Edit history (Undo / Reset / saved looks) is built on **snapshots** in
  ...
  - `baseline` is snapshotted at the end of `openImported` (the per-type auto
    baseline: raw → gray-world WB + auto exposure + measured denoise +
    auto-recover when clipped; camera-rendered → measured denoise only; owner
    ruling 2026-07-25 rev. 2) — that's the Reset target. Hold: Untouched
    renders the fully-neutral params for the bare-decode compare. Reset settles current edits
...
72 modules. Each line is that file's own opening comment, so this
...
- **`src/decode.worker.ts`** (51 lines) — Decoding, off the main thread.
- **`src/decodeClient.ts`** (255 lines) — Main-thread side of the decode workers.
...
- **`src/gl.ts`** (1912 lines) — WebGL2 edit pipeline.
...
- **`src/import.ts`** (114 lines) — Hardened image import.
...
- **`src/look.ts`** (288 lines) — Shareable looks. A look is the CREATIVE grade only (see SavedLook) — small enough (~0.5 KB of JSON) to travel as a link fragment, a paste-able code, or a tiny .ipslook file, with no server and no acco
...
- **`src/lut.ts`** (50 lines) — 3D LUT (.cube) export.
...
- **`src/main.ts`** (14209 lines) — THE INFRARED EDITOR: its whole screen, its whole state, and the orchestration between them.
- **`src/palette.ts`** (118 lines) — Palette family picker, shared across all three pages.
- **`src/pipeline.ts`** (1585 lines) — CPU version of the GPU edit pipeline, kept numerically identical to the fragment shader in gl.ts so exports match the on-screen preview exactly.
...
- **`src/skymap.ts`** (206 lines) — The sky's colour, smoothed AFTER the look has amplified it — a small map rebuilt per edit, blended back in by the sky's own selection.
...
- **`src/zip.ts`** (297 lines) — Minimal ZIP reader — no dependencies.
