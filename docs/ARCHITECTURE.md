...
- **`src/debug.ts`** (1513 lines) — The test page behind the version number.
...
- **`src/gl.ts`** (2354 lines) — WebGL2 edit pipeline.
...
- **`src/main.ts`** (17565 lines) — THE INFRARED EDITOR: its whole screen, its whole state, and the orchestration between them.
...
- **`src/pipeline.ts`** (2346 lines) — CPU version of the GPU edit pipeline, kept numerically identical to the fragment shader in gl.ts so exports match the on-screen preview exactly.
