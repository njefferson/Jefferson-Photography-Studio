...
79 modules. Each line is that file's own opening comment, so this
...
- **`src/keepfile.ts`** (180 lines) — THE KEEP FILE: a photograph the reader owns, as one file they hold.
- **`src/keepstore.ts`** (328 lines) — KEEPING ONE PHOTOGRAPH, ON PURPOSE, UNDER A NAME (decision 039).
...
- **`src/main.ts`** (16693 lines) — THE INFRARED EDITOR: its whole screen, its whole state, and the orchestration between them.
...
- **`src/pipeline.ts`** (2111 lines) — CPU version of the GPU edit pipeline, kept numerically identical to the fragment shader in gl.ts so exports match the on-screen preview exactly.
