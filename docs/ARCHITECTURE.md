...
- **`src/gl.ts`** (1813 lines) — WebGL2 edit pipeline.
...
- **`src/look.ts`** (277 lines) — Shareable looks. A look is the CREATIVE grade only (see SavedLook) — small enough (~0.5 KB of JSON) to travel as a link fragment, a paste-able code, or a tiny .ipslook file, with no server and no acco
...
- **`src/main.ts`** (13998 lines) — THE INFRARED EDITOR: its whole screen, its whole state, and the orchestration between them.
- **`src/palette.ts`** (118 lines) — Palette family picker, shared across all three pages.
- **`src/pipeline.ts`** (1446 lines) — CPU version of the GPU edit pipeline, kept numerically identical to the fragment shader in gl.ts so exports match the on-screen preview exactly.
