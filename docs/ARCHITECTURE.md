...
68 modules. Each line is that file's own opening comment, so this
...
- **`src/export.ts`** (770 lines) — Full-resolution export.
- **`src/export.worker.ts`** (60 lines) — ONE BAND OF AN EXPORT, ON ANOTHER CORE.
- **`src/exportparallel.ts`** (240 lines) — AN EXPORT, SPLIT ACROSS CORES.
- **`src/framecache.ts`** (139 lines) — What the lens rig has already measured, so an interrupted run is not thrown away.
- **`src/gl.ts`** (1686 lines) — WebGL2 edit pipeline.
...
- **`src/look.ts`** (269 lines) — Shareable looks. A look is the CREATIVE grade only (see SavedLook) — small enough (~0.5 KB of JSON) to travel as a link fragment, a paste-able code, or a tiny .ipslook file, with no server and no acco
...
- **`src/main.ts`** (13618 lines) — THE INFRARED EDITOR: its whole screen, its whole state, and the orchestration between them.
- **`src/palette.ts`** (118 lines) — Palette family picker, shared across all three pages.
- **`src/pipeline.ts`** (1330 lines) — CPU version of the GPU edit pipeline, kept numerically identical to the fragment shader in gl.ts so exports match the on-screen preview exactly.
...
- **`src/raw/denoise.ts`** (192 lines) — Edge-preserving denoise (5x5 bilateral) on LINEAR sensor data.
