...
- **`src/keepfile.ts`** (230 lines) — THE KEEP FILE: a photograph the reader owns, as one file they hold.
...
- **`src/main.ts`** (16655 lines) — THE INFRARED EDITOR: its whole screen, its whole state, and the orchestration between them.
