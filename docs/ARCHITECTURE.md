...
- **`src/gl.ts`** (1708 lines) — WebGL2 edit pipeline.
...
- **`src/main.ts`** (13809 lines) — THE INFRARED EDITOR: its whole screen, its whole state, and the orchestration between them.
- **`src/palette.ts`** (118 lines) — Palette family picker, shared across all three pages.
- **`src/pipeline.ts`** (1340 lines) — CPU version of the GPU edit pipeline, kept numerically identical to the fragment shader in gl.ts so exports match the on-screen preview exactly.
...
- **`src/raw/denoise.ts`** (238 lines) — Edge-preserving denoise (5x5 bilateral) on LINEAR sensor data.
