...
- **`src/debug.ts`** (1597 lines) — The test page behind the version number.
...
- **`src/main.ts`** (17684 lines) — THE INFRARED EDITOR: its whole screen, its whole state, and the orchestration between them.
...
- **`src/swupdate.ts`** (285 lines) — THE STANDING "A NEW VERSION IS WAITING" STRIP, for all three PWAs (Doctrine §7h).
