...
73 modules. Each line is that file's own opening comment, so this
...
- **`src/export.ts`** (817 lines) — Full-resolution export.
...
- **`src/gl.ts`** (2030 lines) — WebGL2 edit pipeline.
...
- **`src/main.ts`** (14721 lines) — THE INFRARED EDITOR: its whole screen, its whole state, and the orchestration between them.
- **`src/palette.ts`** (118 lines) — Palette family picker, shared across all three pages.
- **`src/pipeline.ts`** (1725 lines) — CPU version of the GPU edit pipeline, kept numerically identical to the fragment shader in gl.ts so exports match the on-screen preview exactly.
